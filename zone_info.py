...
for name in zone_name:
    ...
    for i in range(len(data_row)):
        if data_len == 4:
            ...
            sql = 'INSERT INTO sigungus (province_id, code, name, latitude, longitude) SELECT %s, %s, %s, %s, %s ' \
                  'FROM DUAL WHERE NOT EXISTS' \
                  '(SELECT province_id, code, name, latitude, longitude FROM sigungus WHERE code = %s AND name = %s)'
